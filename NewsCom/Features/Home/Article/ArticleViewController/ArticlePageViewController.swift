...
extension ArticlePageViewController:  UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        ...
        guard let index = pages.firstIndex(of: viewController as! FullArticleViewController), index > 0 else {
            return nil
        }
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        guard let index = pages.firstIndex(of: viewController as! FullArticleViewController), index < (pages.count - 1) else {
            return nil
        }
        return pages[index + 1]
    }
