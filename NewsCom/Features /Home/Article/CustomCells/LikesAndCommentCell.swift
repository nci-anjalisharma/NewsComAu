...
class LikesAndCommentCell: UITableViewCell {
    ...
    private func setupUI(){
        ...
        likesCountLabel.textColor = theme.colors.textColor
        ...
        commentImageButton.tintColor = theme.colors.primary
        
        commentCountLabel.text = "0"
        commentCountLabel.textColor = theme.colors.textColor
        ...
        moreButton.tintColor = theme.colors.primary
    ...
    private func configureMenu() {
        ...
        let notInterested = UIAction(title: "Not Interested", image: UIImage(systemName: "hand.thumbsdown")) { _ in
            
        }

        let hideArticle = UIAction(title: "Hide Topic", image: UIImage(systemName: "eye.slash")
        ) { _ in
         
        }

        let reportArticle = UIAction(title: "Block Topic", image: UIImage(systemName: "flag"), attributes: .destructive) { _ in
      
        }

        moreButton.menu = UIMenu(title: "", children: [notInterested, hideArticle, reportArticle])
    ...
    func configure(with article: Article) {
        let comments = CommentsManager.shared.comments(for: article.url ?? "")
